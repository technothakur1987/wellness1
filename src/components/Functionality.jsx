...
{/*get data by Date*/}
  ...
  const getDataByDateAndType = (e) => {
    const filteredData = allData.filter((item) => {
      return item.type === selectedType && (formatDate1(item.date)) === (selectedDate);
    });
    dispatch({ type: "SET_FILTERDATA", payload: filteredData });
  };
  ...
  const getDataByTypeAndTitle = (e) => {
    setTimeout(()=>{
        const filteredData = allData.filter((item) => {
            return  item.title.toLowerCase().includes(selectedTitle.toLowerCase()) && item.type === selectedType
          });
        dispatch({ type: "SET_FILTERDATA", payload: filteredData });
  ...
  const getDataByDateAndTypeAndTitle = (e)=>{
    setTimeout(()=>{
        const filteredData = allData.filter((item) => {
            return  item.title.toLowerCase().includes(selectedTitle.toLowerCase()) && (formatDate1(item.date)) === (selectedDate) && item.type === selectedType
          });
      ...
      <div className="flex flex-col md:flex-row basis-full md:basis-[30%] items-stretch md:items-center justify-start   ">
        ...
        <div className=" basis-full md:basis-[30%]  ml-0 md:ml-5 my-3 md:my-0">
          ...
          >
            ...
            {types.map((type) => {
              return (
                <option value={`${type}`} key={type} className="bg-[#efefef] text-black">
                  {type}
                </option>
              );
            })}
